...
as X3DH, the Double Ratchet, Kademlia routing, and Loopix-style mixnets, the primary contribution
lies in their integration into a decentralized hybrid post-quantum messaging architecture
together with a unified game-based treatment of post-quantum forward secrecy, PCS recovery,
and RNG-compromise freshness.
...
Hybrid Post-Quantum Forward Secrecy (PQ-FS) and Post-Compromise Security (PCS).
...
\]
passed through HKDF-SHA3-256, which we model as a pseudorandom function (PRF).  Under this
model, following the analysis of Giacon et al.\ \cite{giacon2018kem}, the concatenation
combiner ensures that $SS_{\text{hybrid}}$ is pseudorandom if either $SS_{cl}$ or the
combination of $SS_{PQ1}$ and $SS_{PQ2}$ remains secure under the stated assumptions; more
precisely, the combiner's IND-CCA2 advantage is bounded by the sum of the component
advantages.  HKDF is modeled as a PRF in all proof sketches (see Assumption~\ref{asm:hkdf} in
Section~\ref{sec:secmodel}).  Our security claims rely specifically on the concatenation-combiner
setting analyzed in \cite{giacon2018kem} and do not necessarily generalize to arbitrary hybrid
compositions.
...
\begin{enumerate}
  \item \textbf{Protocol design:} A fully specified hybrid PQ-X3DH handshake and PQ Double
        Ratchet for asynchronous P2P messaging over a Kademlia DHT, with authenticated
        one-time pre-key bundles (including hash-binding of all OTPK batches in the signed
        payload, closing key substitution and identity misbinding attacks \cite{blake1997unknown}).
  \item \textbf{Formal security model:} Game-based definitions for PQ-FS and PCS adapted from
        \cite{cohngordon2016,dowling2020ratchet} to the hybrid post-quantum setting, with an
        explicit RevealRNG oracle integrated into the freshness definition.
  ...
  \item \textbf{Metadata resistance:} Analysis of the mixnet overlay achieving
        sender-receiver unlinkability under standard mixnet assumptions \cite{loopix}.
...
The game-based arguments in Sections~\ref{sec:security} should be interpreted as structured
proof sketches rather than fully mechanized or reduction-tight proofs in a composable framework.
In particular, we do not claim a complete UC-style treatment of the protocol \cite{uc_framework},
nor a full machine-verified analysis of the ratchet state machine.  Our goal is to provide a
precise cryptographic design together with explicit assumptions, adversarial capabilities, and
security-reduction intuition sufficient to support further peer review, implementation,
and formalization work.
...
HKDF is modeled as a two-stage PRF (extract then expand) following Krawczyk \cite{rfc5869} and
Krawczyk's 2010 analysis \cite{krawczyk2010hkdf} (see Assumption~\ref{asm:hkdf} in
...
preventing identity misbinding and Unknown Key-Share (UKS) attacks \cite{blake1997unknown}:
\[
...
$c = \mathsf{AES\text{-}256\text{-}GCM}_{MK_i}(m;\; \mathsf{nonce} = N_s \| \texttt{0}^{76})$.
...
SHA3-256 as a random oracle; the PRF modeling is sufficient for our reductions.
\end{assumption}
...
side-channel-free execution) following a prior compromise event.  A formal treatment of RNG
compromise in the context of cryptographic protocols can be found in
Dodis et al.\ \cite{dodis2013entropic}.  If the RNG is compromised during the PQ ratchet
...
\cite{cohngordon2016,dowling2020ratchet}.  Let $\mathcal{P} = \{P_1, \ldots, P_n\}$ be a set
...
which encapsulation $\calA_{\mathrm{quant}}$ relies on).  This introduces the factor of $1/2$:
\[
...
combiner, instantiated with HKDF as the PRF per Assumption~\ref{asm:hkdf}), if X25519 remains
computationally secure (no CRQC), then $SS_{cl}$ alone is expected to make $SS_{\text{hybrid}}$
pseudorandom under the stated assumptions, providing standard (classical) forward secrecy.  The
ML-KEM component provides additional PQ-FS against a CRQC.  The two components are
...
\noindent This proposition is inherited directly from the Loopix security analysis \cite{loopix}
and we make no new claims about mixnet security.  Star-Mesh's contribution is the integration of
this overlay with the hybrid post-quantum ratchet layer.
...
ML-DSA-65 & 1952 B  & N/A (sig: 3293 B) & N/A  \\
\bottomrule
\end{tabular}
\end{table}
...
(key-routing records and mailbox polling headers).  The per-message overhead is:
\[
...
\cite{bos2018kyber}, triggered at most once every 50 messages. To validate the computational practicality of our protocol, we micro-benchmarked our Rust prototype over 1{,}000 iterations. On a modern workstation, the complete hybrid PQ-X3DH handshake (comprising Alice's initiator computations, Bob's responder computations, and the corresponding KEM encapsulation and decapsulation) completes in an average of $17.93$~ms. The post-quantum ratchet step (comprising key generation, encapsulation, decapsulation, and Root Key mixing) achieves a round-trip latency of $8.73$~ms. These execution times confirm that the computational overhead of hybrid post-quantum operations is highly practical for standard consumer hardware. The dominant computational overhead in Star-Mesh is expected to be the mixnet onion routing (Sphinx packet construction \cite{sphinx}), which requires multiple asymmetric operations per hop and is independent of the PQC primitives.
...
fine-grained model.  Our security model and argument structure build directly on these works,
extending them to the hybrid post-quantum setting with ML-KEM.
...
achieves IND-CCA2 security if either component KEM is IND-CCA2 secure, provided the combiner
uses a suitable KDF modeled as a PRF (as established in \cite{giacon2018kem}).  Our
hybrid KEM construction applies the composition results of \cite{giacon2018kem} with HKDF as
