...
as X3DH, the Double Ratchet, Kademlia routing, and Loopix-style mixnets, the primary novelty
...
Diffie--Hellman with ML-KEM-768~\cite{nist_fips203} under a concatenation combiner secured by
...
$MK_i$ is zeroized immediately after use.
...
\cite{bos2018kyber}, triggered at most once every 50 messages.  The dominant computational
overhead in Star-Mesh is expected to be the mixnet onion routing (Sphinx packet construction
\cite{sphinx}), which requires multiple asymmetric operations per hop and is independent of the
PQC primitives.
...
format \cite{sphinx} provides efficient onion encryption.  Star-Mesh inherits these results
...
We presented Star-Mesh, a \emph{hybrid} post-quantum secure messaging protocol combining a
