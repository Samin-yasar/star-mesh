...
\noindent This proposition is adapted from the quantitative sender-receiver third-party
unlinkability analysis in Section~4.3 of \cite{loopix} (the Poisson-mix model with
constant-rate cover traffic at rate $\lambda_{\mathrm{cover}}$); the bound $\frac{f}{1-f}
\cdot \frac{1}{n_{\mathrm{mix}}}$ follows from that analysis under Assumption~\ref{asm:mixnet}.
We make no new claims about mixnet security.  Star-Mesh's contribution is the integration of
